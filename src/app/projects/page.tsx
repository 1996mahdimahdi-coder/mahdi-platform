...
// Top successful Algerian micro-business picks curated by NABDA
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    fetchProjects();
  }, [selectedCategory, selectedRisk, homeBasedOnly, onlineOnly]);

  const fetchProjects = async () => {
    setLoading(true);
    let url = `/api/projects?category=${encodeURIComponent(selectedCategory)}&risk=${encodeURIComponent(selectedRisk)}&homeBased=${homeBasedOnly}&online=${onlineOnly}`;
    if (maxCapital) {
      url += `&maxCapital=${maxCapital}`;
    }
